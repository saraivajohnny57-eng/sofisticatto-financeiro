...
function abrirBoletoBradescoRegistro(id){
  ...
  const nosso=String(x.nosso_numero||'').trim();
  const linhaFmt=linha.replace(/\s+/g,' ').trim();
  const valor=esc(cobMoeda(x.valor));
  const beneficiario='SOFISTICATTO COSMETICOS';
  const cnpjBenef='05.451.985/0001-95';
  const agenciaCed='2274-8 / 0280052-7';
  const carteira='09';
  const endBenef='RUA 4 N 217 QD 35 LOTE 14E - VILA ABAJA';
  const cidBenef='74550-470 - GOIANIA - GO';
  const numeroDoc=String(x.numero_nf||'—');
  const bloco=`<div class="ficha"><div class="cab"><div class="marca"><b>Bradesco</b></div><div class="banco">237-2</div><div class="ld">${esc(linhaFmt)}</div></div><div class="row"><div class="cell grow"><small>Local de Pagamento</small><b>Pagável preferencialmente na Rede Bradesco e Bradesco Expresso</b></div><div class="cell venc"><small>Vencimento</small><b>${esc(venc)}</b></div></div><div class="row"><div class="cell grow"><small>Beneficiário</small><b>${beneficiario} - CNPJ: ${cnpjBenef}</b></div><div class="cell venc"><small>Agência / Cód. Beneficiário</small><b>${agenciaCed}</b></div></div><div class="row"><div class="cell"><small>Data do documento</small><b>${esc(emissao)}</b></div><div class="cell"><small>Número do documento</small><b>${esc(numeroDoc)}</b></div><div class="cell"><small>Espécie Documento</small><b>DM</b></div><div class="cell"><small>Aceite</small><b>N</b></div><div class="cell"><small>Data Processamento</small><b>${esc(emissao)}</b></div><div class="cell venc"><small>Nosso Número</small><b>${esc(nosso||'Retornado no registro')}</b></div></div><div class="row"><div class="cell"><small>Uso do Banco</small>&nbsp;</div><div class="cell"><small>CIP</small>&nbsp;</div><div class="cell"><small>Carteira</small><b>${carteira}</b></div><div class="cell"><small>Espécie Moeda</small><b>R$</b></div><div class="cell grow"><small>Quantidade / Valor</small>&nbsp;</div><div class="cell venc"><small>1 (=) Valor do Documento</small><b>${valor}</b></div></div><div class="corpo"><div class="instr"><b>*** VALORES EXPRESSOS EM REAIS ***</b><div class="espaco"></div><b>Pagador: ${esc(x.cliente_nome||'—')} - CPF/CNPJ: ${esc(docFmt||'—')}</b><br>${esc(endPag||'—')}<br>${esc(cidPag||'')}<br><br><b>Beneficiário Final:</b> ${beneficiario} - CNPJ: ${cnpjBenef}<br>${endBenef}<br>${cidBenef}</div><div class="valores"><div>2 (-) Desconto / Abatimento</div><div>3 (-) Outras Deduções</div><div>4 (+) Mora Multa</div><div>5 (+) Outros Acréscimos</div><div>6 (=) Valor Cobrado</div></div></div><div class="bar"><svg class="barcode"></svg></div><div class="rod">Autenticação Mecânica &nbsp;&nbsp; <b>Ficha de Compensação</b></div></div>`;
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Boleto Bradesco ${esc(numeroDoc)}</title><style>@page{size:A4 portrait;margin:7mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;margin:0;color:#111;font-size:10px}.aviso{max-width:195mm;margin:0 auto 5mm;padding:7px 9px;background:#f6f6f6;border:1px solid #ccc;font-size:10px}.ficha{max-width:195mm;margin:0 auto;border-bottom:1px dashed #555;padding-bottom:5mm}.cab{height:12mm;display:flex;align-items:center;border-bottom:2px solid #111}.marca{font-size:18px;padding:0 10px 0 3px;border-right:1px solid #111}.marca b:before{content:'▰';color:#c81920;margin-right:5px}.banco{font-size:22px;font-weight:800;padding:0 14px;border-right:1px solid #111}.ld{font-size:14px;font-weight:800;flex:1;text-align:right;padding-right:3px}.row{display:flex;border-bottom:1px solid #555;min-height:10mm}.cell{padding:2px 5px;border-right:1px solid #555;min-width:18mm}.cell:last-child{border-right:0}.cell.grow{flex:1}.cell.venc{width:52mm}.cell small{display:block;font-size:8px}.cell b{font-size:10px}.corpo{display:flex;min-height:72mm;border-bottom:1px solid #555}.instr{flex:1;padding:7px 5px;line-height:1.45}.espaco{height:25mm}.valores{width:52mm;border-left:1px solid #555}.valores div{height:11mm;border-bottom:1px solid #555;padding:3px}.bar{height:22mm;padding:3mm 4mm 0}.barcode{width:105mm;height:15mm}.rod{text-align:right;padding:3px;font-size:9px}.acoes{max-width:195mm;margin:6mm auto}button{padding:7px 14px}@media print{.aviso,.acoes{display:none}.ficha{max-width:none}}</style><script src="https://cdn.jsdelivr.net/npm/jsbarcode@3.11.6/dist/JsBarcode.all.min.js"><\/script></head><body><div class="aviso"><b>Impressão do boleto já registrado.</b> Esta tela não realiza nova emissão. Linha digitável e código de barras são os dados armazenados no Histórico após o retorno do Bradesco.</div>${bloco}<div class="acoes"><button onclick="window.print()">🖨 Imprimir boleto</button></div><script>window.onload=function(){try{document.querySelectorAll('.barcode').forEach(function(el){JsBarcode(el,'${barras44}',{format:'ITF',displayValue:false,height:48,width:1.25,margin:0});});}catch(e){}}<\/script></body></html>`);
  w.document.close();
}
