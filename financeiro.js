...
function obterChaveAdminSmtp(){
  let chave=sessionStorage.getItem("integrations_admin_key")||localStorage.getItem("integrations_admin_key")||"";
  if(chave) return chave;
  chave=prompt("Informe a chave administrativa INTEGRATIONS_ADMIN_KEY para configurar o remetente:")||"";
  chave=chave.trim();
  if(chave) sessionStorage.setItem("integrations_admin_key",chave);
  return chave;
}
async function apiSmtpUsuario(acao,{login="",email="",nome="",senha_app=""}={}){
  const chave=obterChaveAdminSmtp();
  if(!chave) throw new Error("Chave administrativa não informada.");
  const r=await fetch('/api/remetentes-smtp',{method:'POST',headers:{'Content-Type':'application/json','x-integrations-key':chave},body:JSON.stringify({acao,login,email,nome,senha_app})});
  const d=await r.json().catch(()=>({}));
  if(r.status===401){sessionStorage.removeItem('integrations_admin_key');localStorage.removeItem('integrations_admin_key');}
  if(!r.ok||!d.ok) throw new Error(d.erro||'Não foi possível configurar o remetente.');
  return d;
}
async function configurarSmtpUsuario(login,email,nome){
  if(!email){alert('Cadastre primeiro o e-mail remetente deste usuário.');return;}
  const senha=prompt('Cole a senha de aplicativo do Google para '+email+' (os espaços podem ser mantidos):')||'';
  if(!senha.trim()) return;
  try{const d=await apiSmtpUsuario('salvar',{login,email,nome,senha_app:senha});alert(d.mensagem+'\n\nAgora clique em Testar.');carregarUsuarios();}
  catch(e){alert('SMTP: '+e.message);}
}
async function testarSmtpUsuario(login,email,nome){
  try{const d=await apiSmtpUsuario('testar',{login,email,nome});alert(d.mensagem);carregarUsuarios();}
  catch(e){alert('Teste SMTP: '+e.message);}
}
async function desativarSmtpUsuario(login,email){
  if(!confirm('Desativar este remetente? Os envios deste usuário deixarão de usar esta senha de aplicativo.')) return;
  try{const d=await apiSmtpUsuario('desativar',{login,email});alert(d.mensagem);carregarUsuarios();}
  catch(e){alert('SMTP: '+e.message);}
}
async function consultarStatusSmtpUsuario(id,login,email,nome){
  const el=document.getElementById('smtpStatus_'+id); if(el)el.textContent='Consultando...';
  try{const d=await apiSmtpUsuario('status',{login,email,nome});
    if(el) el.innerHTML=d.configurado?(d.ativo?'<span style="color:#16894a;font-weight:700">Configurado</span>':'<span style="color:#b77900;font-weight:700">Desativado</span>'):'<span style="color:#b3261e">Não configurado</span>';
...
async function criarUsuario(){
  ...
  if(email_remetente && senha_app.trim()){
    try{
      const x=await apiSmtpUsuario('salvar',{login,email:email_remetente,nome:nome_exibicao||login,senha_app});
      msg += "\n\nSMTP: "+x.mensagem+" Use o botão Testar para validar o envio.";
    }catch(e){ msg += "\n\nUsuário salvo, mas a senha de aplicativo não foi configurada: "+e.message; }
  }else if(email_remetente){
...
// V138 — Instruções padrão do Banco do Brasil, conforme tela operacional da Sofisticatto.
// Campos que estavam SEM MARCA permanecem sem seleção (null).
const COBRANCA_PADRAO_BB = Object.freeze({
  abatimento:null,
  desconto:null,
  juros:{tipo:"percentual_mes",percentual:5},
  multa:{tipo:"percentual",percentual:2,dias_corridos_apos_vencimento:1},
  pagamento_parcial:null,
  protesto_negativacao:null
});

function instrucoesPadraoBancoCobranca(codigo){
  if(String(codigo||"").toLowerCase()==="bb") return JSON.parse(JSON.stringify(COBRANCA_PADRAO_BB));
  return {abatimento:null,desconto:null,juros:null,multa:null,pagamento_parcial:null,protesto_negativacao:null};
}
...
function atualizarPainelInstrucoesBancoBoleto(){
  ...
  const cod=document.getElementById("bolBancoCodigo")?.value||"";
  if(cod!=="bb"){box.innerHTML="";return;}
  box.innerHTML=`<div class="bol-instrucoes-bb"><b>🇧🇷 Padrão Banco do Brasil</b><span>Abatimento: sem marca</span><span>Desconto: sem marca</span><span>Juros: 5,00% ao mês</span><span>Multa: 2,00% • 1 dia corrido após o vencimento</span><span>Pagamento parcial: sem marca</span><span>Protesto/Negativação: sem marca</span></div>`;
}
...
function abrirBoletoBradescoRegistro(id){
  ...
  const barras44=String(x.codigo_barras||'').replace(/\D/g,'').length===44?String(x.codigo_barras).replace(/\D/g,''):bradescoCodigoBarras44DaLinha(linha);
  if(barras44.length!==44)return alert('O código de barras oficial deste boleto não está completo no Histórico. Nenhuma nova emissão será feita.');
  ...
  const fmtData=v=>v?new Date(String(v).slice(0,10)+'T12:00:00').toLocaleDateString('pt-BR'):'—';
  const emissao=fmtData(x.emitido_em||x.created_at), venc=fmtData(x.vencimento);
  const doc=String(x.cpf_cnpj||'').replace(/\D/g,'');
  const docFmt=doc.length===14?doc.replace(/^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/,'$1.$2.$3/$4-$5'):doc.length===11?doc.replace(/^(\d{3})(\d{3})(\d{3})(\d{2})$/,'$1.$2.$3-$4'):doc;
  const cep=String(x.cep||'').replace(/\D/g,'').replace(/^(\d{5})(\d{3})$/,'$1-$2');
  const endPag=[x.endereco,x.numero].filter(Boolean).join(' ');
  const cidPag=[cep,x.cidade,x.uf].filter(Boolean).join(' - ');
  // V256: o Histórico antigo pode não ter salvo nosso_numero, embora a linha digitável oficial tenha sido armazenada.
  // No Bradesco, o campo livre do código de barras contém: agência(4)+carteira(2)+nosso número base(11)+beneficiário(7)+0.
  // O DV do Nosso Número é módulo 11 sobre carteira + base, com pesos 2..7 da direita para a esquerda.
  function dvNossoNumeroBradesco(carteira2, base11){
    const src=String(carteira2||'').replace(/\D/g,'')+String(base11||'').replace(/\D/g,'');
    let soma=0,peso=2;
    for(let i=src.length-1;i>=0;i--){ soma+=Number(src[i])*peso; peso= peso===7 ? 2 : peso+1; }
    const v=11-(soma%11);
    return v===10?'P':(v===11?0:v);
  }
  function nossoNumeroBradescoExibicao(reg, barras){
    const salvo=String(reg?.nosso_numero||'').trim();
    // Se já houver um Nosso Número formatado/legado, preserva-o.
    if(salvo && salvo!=='—'){
      const dig=salvo.replace(/\D/g,'');
      if(dig.length>=11){
        // nuTituloGerado da API pode vir apenas com a base de 11 dígitos.
        const base=dig.slice(-11), cart='09';
        return `${cart} / ${base}-${dvNossoNumeroBradesco(cart,base)}`;
      }
      return salvo;
    }
    const b=String(barras||'').replace(/\D/g,'');
    if(b.length===44){
      const cart=b.slice(23,25), base=b.slice(25,36);
      if(/^\d{2}$/.test(cart)&&/^\d{11}$/.test(base)) return `${cart} / ${base}-${dvNossoNumeroBradesco(cart,base)}`;
    }
    return 'Não identificado';
  }
  const nosso=nossoNumeroBradescoExibicao(x,barras44);
  const linhaFmt=linha.replace(/\s+/g,' ').trim();
  const valor=esc(cobMoeda(x.valor));
  ...
  const ficha=(titulo,comBarcode)=>`<section class="ficha">${cab()}<div class="tipo">${titulo}</div><div class="row"><div class="cell grow"><small>Local de Pagamento</small><b>Pagável preferencialmente na Rede Bradesco e Bradesco Expresso</b></div><div class="cell venc"><small>Vencimento</small><b>${esc(venc)}</b></div></div><div class="row"><div class="cell grow"><small>Beneficiário</small><b>${beneficiario} - CNPJ: ${cnpjBenef}</b></div><div class="cell venc"><small>Agência / Cód. Beneficiário</small><b>${agenciaCed}</b></div></div><div class="row"><div class="cell"><small>Data do documento</small><b>${esc(emissao)}</b></div><div class="cell"><small>Número do documento</small><b>${esc(numeroDoc)}</b></div><div class="cell"><small>Espécie Documento</small><b>DM</b></div><div class="cell"><small>Aceite</small><b>N</b></div><div class="cell"><small>Data Processamento</small><b>${esc(emissao)}</b></div><div class="cell venc"><small>Nosso Número</small><b>${esc(nosso||'—')}</b></div></div><div class="row"><div class="cell"><small>Uso do Banco</small>&nbsp;</div><div class="cell"><small>CIP</small>&nbsp;</div><div class="cell"><small>Carteira</small><b>${carteira}</b></div><div class="cell"><small>Espécie Moeda</small><b>R$</b></div><div class="cell grow"><small>Quantidade / Valor</small>&nbsp;</div><div class="cell venc"><small>1 (=) Valor do Documento</small><b>${valor}</b></div></div><div class="corpo"><div class="instr"><b>*** VALORES EXPRESSOS EM REAIS ***</b><div class="espaco"></div><b>Pagador: ${esc(x.cliente_nome||'—')} - CPF/CNPJ: ${esc(docFmt||'—')}</b><br>${esc(endPag||'—')}<br>${esc(cidPag||'')}<br><br><b>Beneficiário Final:</b> ${beneficiario} - CNPJ: ${cnpjBenef}<br>${endBenef}<br>${cidBenef}</div><div class="valores"><div>2 (-) Desconto / Abatimento</div><div>3 (-) Outras Deduções</div><div>4 (+) Mora Multa</div><div>5 (+) Outros Acréscimos</div><div>6 (=) Valor Cobrado</div></div></div>${comBarcode?`<div class="bar"><svg class="barcode"></svg></div><div class="rod">Autenticação Mecânica &nbsp;&nbsp; <b>Ficha de Compensação</b></div>`:`<div class="rod">Autenticação Mecânica</div>`}</section>`;
  const original=String(x.pdf_url||'').trim();
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Boleto Bradesco ${esc(numeroDoc)}</title><style>@page{size:A4 portrait;margin:6mm}*{box-sizing:border-box}body{font-family:Arial,Helvetica,sans-serif;margin:0;color:#111;font-size:9px}.aviso,.acoes{max-width:195mm;margin:0 auto 4mm;padding:6px 8px;background:#f6f6f6;border:1px solid #ccc}.acoes{background:#fff;border:0;padding-left:0}.acoes button,.acoes a{display:inline-block;padding:7px 12px;margin-right:6px;border:1px solid #999;background:#fff;color:#111;text-decoration:none;cursor:pointer}.canhoto,.ficha{position:relative;max-width:195mm;margin:0 auto 3mm}.canhoto{border-bottom:1px dashed #555;padding-bottom:3mm}.cab{height:10mm;display:flex;align-items:center;border-bottom:1.5px solid #111}.logo-bradesco{width:35mm;height:auto;max-height:8mm;object-fit:contain;object-position:left center;padding-right:3mm;border-right:1px solid #111}.banco{font-size:20px;font-weight:800;padding:0 12px;border-right:1px solid #111}.ld{font-size:12px;font-weight:800;flex:1;text-align:right;padding-right:3px}.tipo{position:absolute;right:3px;top:11mm;font-weight:bold;font-size:8px;background:#fff;padding-left:3px}.stubgrid{display:grid;grid-template-columns:2fr 1fr 2.4fr;border-bottom:1px solid #555}.stubgrid>div{padding:1.4mm 1.2mm;border-right:1px solid #555;border-bottom:1px solid #777;min-height:8mm;overflow:hidden}.stubgrid small,.cell small,.motivos small{display:block;font-size:6.5px;line-height:1.1}.stubgrid b,.cell b{display:block;font-size:8px;line-height:1.15;overflow-wrap:anywhere}.motivos{grid-row:1 / span 3;grid-column:3;display:flex;flex-direction:column;gap:1.2mm}.motivos span{font-size:6.5px;line-height:1.15}.recebe{padding:1.5mm 1mm;border-bottom:1px solid #555;white-space:normal}.local{padding:1.3mm 1mm;min-height:6mm}.local span{float:right}.row{display:flex;border-bottom:1px solid #555;min-height:8.5mm;align-items:stretch}.cell{padding:1.2mm 1mm;border-right:1px solid #555;min-width:18mm;overflow:hidden}.cell:last-child{border-right:0}.cell.grow{flex:1;min-width:0}.cell.venc{width:52mm;flex:0 0 52mm}.corpo{display:flex;min-height:55mm;border-bottom:1px solid #555}.instr{flex:1;min-width:0;padding:2mm 1mm;line-height:1.35}.espaco{height:16mm}.valores{width:52mm;flex:0 0 52mm;border-left:1px solid #555}.valores div{height:9mm;border-bottom:1px solid #555;padding:1.5mm 1mm}.bar{height:19mm;padding:2mm 4mm 0}.barcode{width:105mm;height:14mm}.rod{text-align:right;padding:2px;font-size:8px}.corte{max-width:195mm;margin:1mm auto 2mm;border-top:1px dashed #555;text-align:right;font-size:7px;padding-top:1px}@media print{.aviso,.acoes{display:none}.canhoto,.ficha{max-width:none}.corte{max-width:none}}</style><script src="https://cdn.jsdelivr.net/npm/jsbarcode@3.11.6/dist/JsBarcode.all.min.js"><\/script></head><body><div class="aviso"><b>V256 — boleto registrado no Bradesco.</b> Esta tela não realiza nova emissão. O layout usa os dados oficiais armazenados no Histórico. ${original?'Também existe um endereço de boleto/PDF retornado pelo banco.':'A resposta atual do banco não trouxe URL/PDF original; por isso a via abaixo é montada com os dados oficiais retornados.'}</div>${canhoto}<div class="corte">✂ corte/destaque</div>${ficha('Recibo do Pagador',false)}<div class="corte">✂ corte/destaque</div>${ficha('Ficha de Compensação',true)}<div class="acoes"><button onclick="window.print()">🖨 Imprimir boleto completo</button>${original?`<a href="${esc(original)}" target="_blank" rel="noopener">🏦 Abrir boleto original retornado pelo Bradesco</a>`:''}</div><script>window.onload=function(){try{document.querySelectorAll('.barcode').forEach(function(el){JsBarcode(el,'${barras44}',{format:'ITF',displayValue:false,height:48,width:1.25,margin:0});});}catch(e){console.error(e)}}<\/script></body></html>`);
  w.document.close();
}
...
async function abrirEmissaoBoletoRelatorio(relatorioId){
  ...
  const jurosPadrao=bancoCodigoAtual==="bb"?5:0;
  document.getElementById("bolMulta").value=String(boletoCobrancaAtual?.multa_percentual??multaPadrao).replace(".",",");
  document.getElementById("bolJuros").value=String(boletoCobrancaAtual?.juros_percentual??jurosPadrao).replace(".",",");
  atualizarPainelInstrucoesBancoBoleto();
...
async function salvarPreparacaoBoletoRelatorioV175(){
  ...
  const comuns={
    ...
    instrucoes_banco:instrucoesPadraoBancoCobranca(codigoBancoCobranca(boletoRelatorioAtual.banco)),
...
async function emitirGrupoBoletoRelatorioReal(rows,ctx={}){
  ...
  if(st){
    ...
    if(emitidos.length)st.innerHTML+=`<div class="bol-retorno-acoes"><button type="button" class="btn azul" onclick="imprimirResultadosBbUltimaEmissao()">🖨 Impressão Normal${emitidos.length>1?' • '+emitidos.length+' páginas':''}</button><button type="button" class="btn verde" onclick="abrirSegundasViasBbUltimaEmissao()">🏦 2ª via BB</button></div>`;
  }
...
async function gerarPdfBoletoBb(d,opt={}){
  ...
  const pg=pdf.addPage([595.28,841.89]); // A4
  const font=await pdf.embedFont(StandardFonts.Helvetica),bold=await pdf.embedFont(StandardFonts.HelveticaBold);
  const W=595.28,H=841.89;
  const L=28,R=28,CW=W-L-R;
  ...
  const cell=(x,y,w,h,label,value,opt={})=>{
    rect(x,y,w,h,opt.borderWidth??.45);
    drawTextFit(label,x+3,y+h-7,w-6,opt.labelSize||4.8,font);
  ...
  // Logo da Sofisticatto: fica fora da área padronizada do boleto.
  ...
  async function embedPngAsset(path){
    try{const u=new URL(path,window.location.href).href;const r=await fetch(u,{cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);return await pdf.embedPng(new Uint8Array(await r.arrayBuffer()));}
  ...
  async function drawLogoSofisticatto(x,y,w,h){
    ...
    drawTextFit('Sofisticatto',x,y+h/2-4,w,15,bold,'center');drawTextFit('COSMÉTICOS',x,y+3,w,5,font,'center');return false;
  }
  function bankHeader(yTop){
    // Especificação BB: nome/logo da IF à esquerda; 001-9; linha digitável à direita.
    const h=mm(12.5);
    ...
    drawTextFit('001-9',L+bankW+2,yTop-h+10,codeW-4,14,bold,'center');
    line(L+bankW+codeW,yTop-h,L+bankW+codeW,yTop,.75);
    drawTextFit(linhaFmt,L+bankW+codeW+5,yTop-h+11,CW-bankW-codeW-10,10.2,bold,'center');
    return yTop-h;
  }

  // ===========================
  // CABEÇALHO CORPORATIVO (fora do boleto)
  // ===========================
  let y=817;
  // Na Impressão Normal o boleto começa diretamente pelo padrão bancário,
  // sem logomarca/cabeçalho da Sofisticatto, como no BB Cobrança.
  if(!opt.impressaoNormal){
    await drawLogoSofisticatto(L,y-53,172,48);
    drawTextFit(beneficiario,L+190,y-18,CW-190,7.4,bold);
    drawTextFit('CNPJ: '+cnpjBen,L+190,y-32,CW-190,6,font);
    drawTextFit(enderecoBen,L+190,y-45,CW-190,5.5,font);
    line(L,y-58,L+CW,y-58,.65);
    y=747;
  }

  // ===========================
  // CANHOTO / RECIBO DE ENTREGA (V169)
  // Layout redesenhado para reproduzir a divisão solicitada:
  // cabeçalho bancário + título dentro da própria moldura,
  // duas linhas de identificação, linha financeira em 5 colunas,
  // linha de recebimento/assinatura/data/nome e faixa de pagamento.
  // ===========================
  if(opt.impressaoNormal){
    const top=y;
    const headH=42;
    ...
    let ry=top-headH;
    const rightIdW=180;
    const leftIdW=CW-rightIdW;
    const xId=L+leftIdW;

    // Beneficiário / Agência.
    rect(L,ry-rowIdH,CW,rowIdH,.55);
    line(xId,ry-rowIdH,xId,ry,.5);
    drawTextFit('Nome do Beneficiário/CNPJ/CPF',L+6,ry-8,leftIdW-12,4.4,font);
    drawTextFit(`${beneficiario} - CNPJ: ${cnpjBen}`,L+6,ry-22,leftIdW-12,5.6,font);
    drawTextFit('Agência / Código do Beneficiário',xId+6,ry-8,rightIdW-12,4.4,font);
    drawTextFit(agenciaCodigo,xId+6,ry-22,rightIdW-12,6.2,font,'right');
    ry-=rowIdH;

    // Pagador / Nosso-Número.
    rect(L,ry-rowIdH,CW,rowIdH,.55);
    line(xId,ry-rowIdH,xId,ry,.5);
    drawTextFit('Pagador/CNPJ/CPF',L+6,ry-8,leftIdW-12,4.4,font);
    drawTextFit(`${d.cliente_nome||''} - CNPJ/CPF: ${d.cpf_cnpj||''}`,L+6,ry-22,leftIdW-12,5.8,font);
    drawTextFit('Nosso-Número',xId+6,ry-8,rightIdW-12,4.4,font);
    drawTextFit(d.nosso_numero||'—',xId+6,ry-22,rightIdW-12,6.4,font,'right');
    ry-=rowIdH;

    // Linha financeira: 5 células em toda a largura do canhoto.
    const finWidths=[108,96,66,58,CW-328];
    const finData=[
      ['Data de Vencimento',venc],
      ['Nr Documento',d.numero_titulo||'—'],
      ['Espécie','DM'],
      ['Moeda','R$'],
      ['Valor do Documento',moeda]
    ];
    rect(L,ry-rowFinH,CW,rowFinH,.55);
    let fx=L;
    finData.forEach((item,i)=>{
      const w=finWidths[i];
      if(i>0)line(fx,ry-rowFinH,fx,ry,.5);
      drawTextFit(item[0],fx+5,ry-9,w-10,4.7,font,i===4?'left':'center');
      drawTextFit(item[1],fx+5,ry-rowFinH+6,w-10,i===4?8.0:7.0,i===4?bold:font,i===4?'right':'center');
      fx+=w;
    });
    ry-=rowFinH;

    // Recebimento / assinatura / data / nome.
    const recWidths=[188,157,92,CW-437];
    const recLabels=[
      'Recebi(emos) o boleto com essas características.',
      'Assinatura',
      'Data da Entrega',
      'Nome'
    ];
    rect(L,ry-rowRecH,CW,rowRecH,.55);
    let rx=L;
    recLabels.forEach((label,i)=>{
      const w=recWidths[i];
      if(i>0)line(rx,ry-rowRecH,rx,ry,.5);
      drawTextFit(label,rx+6,ry-17,w-12,i===0?5.0:5.2,font,'left');
      rx+=w;
    });
    ry-=rowRecH;

    // Faixa inferior — Local de Pagamento.
    rect(L,ry-payH,CW,payH,.55);
    drawTextFit('Local de Pagamento',L+7,ry-10,CW-14,5.0,font);
    drawTextFit('Pagável em qualquer banco.',L+7,ry-26,CW-14,7.4,bold);
    ry-=payH;

    // Linha de corte — somente abaixo do canhoto, sem texto lateral ou elementos sobrepostos.
    const cutStubY=ry-14;
    drawTextFit('Corte aqui',L+4,cutStubY+5,54,4.8,font);
    line(L+60,cutStubY,L+CW,cutStubY,.55,[6,5]);
    y=cutStubY-18;
  }
  ...
  // ===========================
  drawTextFit('Recibo do Pagador',L+CW-120,y+8,120,6.4,bold,'right');
  y=bankHeader(y);
  const reciboTop=y;
  ...
  // Linha pontilhada recomendada entre Recibo do Pagador e Ficha de Compensação.
  const cutY=374;
  line(L,cutY,L+CW,cutY,.55,[5,4]);
  drawTextFit('Corte aqui',L+3,cutY+6,58,4.7,font);
...
async function mesclarPdfsBb(blobs){
  await carregarScriptUmaVez('https://cdn.jsdelivr.net/npm/pdf-lib@1.17.1/dist/pdf-lib.min.js','PDFLib');
...
function registrosGrupoImpressaoNormalBb(reg){
  const todos=[...(cobrancasBancarias||[]),...(cobrancaMassaPreparadas||[])];
  const mapa=new Map();
  for(const x of todos){if(x?.id!=null)mapa.set(String(x.id),x)}
  const unicos=[...mapa.values()];
  let grupo=[];
  if(reg?.relatorio_id!=null&&String(reg.relatorio_id)!==''){
    grupo=unicos.filter(x=>String(x.relatorio_id)===String(reg.relatorio_id));
  }
  if(!grupo.length){
    const doc=String(reg?.cpf_cnpj||'').replace(/\D/g,''), nf=String(reg?.numero_nf||'').trim();
    grupo=unicos.filter(x=>String(x?.cpf_cnpj||'').replace(/\D/g,'')===doc&&String(x?.numero_nf||'').trim()===nf);
  }
  if(!grupo.length)grupo=[reg];
  return grupo.filter(x=>codigoBancoCobranca(x?.banco_nome||x?.banco||'bb')==='bb'&&x?.status!=='cancelado')
    .sort((a,b)=>Number(a.parcela_numero||1)-Number(b.parcela_numero||1));
}
async function gerarPdfImpressaoNormalBb(registros){
  const lista=(registros||[]).filter(Boolean).sort((a,b)=>Number(a.parcela_numero||1)-Number(b.parcela_numero||1));
  if(!lista.length)throw new Error('Nenhum boleto encontrado para impressão.');
  const blobs=[];
  for(const reg of lista){
    const d=reg?.registroParaVisual||registroParaBoletoVisual(reg,reg?.emissao||{});
    blobs.push(await gerarPdfBoletoBb(d,{impressaoNormal:true}));
  }
  return mesclarPdfsBb(blobs);
}
...
async function abrirImpressaoNormalBbRegistro(id){
  ...
  try{
    const grupo=registrosGrupoImpressaoNormalBb(x);
    const blob=await gerarPdfImpressaoNormalBb(grupo);
    const url=URL.createObjectURL(blob);
    window.open(url,'_blank');
    setTimeout(()=>URL.revokeObjectURL(url),120000);
  }catch(e){alert('Não foi possível gerar a Impressão Normal.\n\n'+e.message)}
}
...
async function imprimirResultadosBbUltimaEmissao(){
  const itens=(window.__bbUltimosResultados||[]).map(x=>{
    const reg=x.registro||x.p||{};
    return {...reg,emissao:x.emissao||{},registroParaVisual:registroParaBoletoVisual(reg,x.emissao||{})};
  });
  if(!itens.length)return alert('Nenhum boleto disponível para impressão.');
  try{
    const blob=await gerarPdfImpressaoNormalBb(itens);
    const url=URL.createObjectURL(blob);
    window.open(url,'_blank');
    setTimeout(()=>URL.revokeObjectURL(url),120000);
  }catch(e){alert('Não foi possível gerar a Impressão Normal.\n\n'+e.message)}
}
async function abrirSegundasViasBbUltimaEmissao(){
  const itens=(window.__bbUltimosResultados||[]);
  if(!itens.length)return alert('Nenhum boleto disponível.');
  // Abre uma página inicial imediatamente para evitar bloqueio de pop-up no primeiro título.
  for(let i=0;i<itens.length;i++){
    const x=itens[i],reg=x.registro||x.p;
    try{
      const url=await obterUrlBoletoOficialBb(reg,x.emissao||{});
      if(i===0)window.open(url||'https://boleto.apps.bb.com.br/segunda-via-final','_blank');
      else if(url)window.open(url,'_blank');
    }catch(e){alert('2ª via BB indisponível: '+e.message);break}
  }
}
async function salvarResultadosBbUltimaEmissao(){
  const itens=(window.__bbUltimosResultados||[]).map(x=>{
    const reg=x.registro||x.p||{};
    return {...reg,emissao:x.emissao||{},registroParaVisual:registroParaBoletoVisual(reg,x.emissao||{})};
  });
  ...
  try{
    const blob=await gerarPdfImpressaoNormalBb(itens);
    const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=nomePdfImpressaoNormalBb(itens);a.click();
    setTimeout(()=>URL.revokeObjectURL(a.href),10000);
  }catch(e){alert('Não foi possível salvar a Impressão Normal.\n\n'+e.message)}
}
async function emitirBoletoBancoIntegradoV155(p,g){
  ...
  // V165: blob de Impressão Normal, sem logomarca corporativa, para composição em PDF único.
  const blob=await gerarPdfBoletoBb(d,{impressaoNormal:true});return {...r.emissao,urlImagemBoleto:urlOficial||r.emissao?.urlImagemBoleto||null,blob,registro:reg};
}
...
async function carregarFilaCobrancaMassa(){
  ...
  try{
    const [rel,prep]=await Promise.all([
      banco.from("boletos").select("*").neq("status","Finalizado").order("valor",{ascending:false}),
      banco.from("cobrancas_bancarias").select("*").neq("status","cancelado").order("parcela_numero",{ascending:true})
    ]);
    ...
    if(tb)tb.innerHTML=`<tr><td colspan="7">⚠ ${escaparHtmlEmail(e.message||String(e))}</td></tr>`;
  }
}
...
function renderFilaCobrancaMassa(){
  const tb=document.getElementById("cobMassaTabela");if(!tb)return;
  const grupos=(cobrancaMassaRelatorios||[]).map(dadosGrupoCobrancaMassa).sort((a,b)=>b.valorTotal-a.valorTotal);
  tb.innerHTML=grupos.length?grupos.map(g=>{
    let situacao="";
    ...
    return `<tr><td>${g.pronto?`<input type="checkbox" class="cob-massa-check" data-id="${g.rel.id}" onchange="atualizarResumoCobrancaMassa()">`:''}</td><td><b>${escaparHtmlEmail(g.rel.nome||'')}</b></td><td>${escaparHtmlEmail(g.rel.banco||'—')}</td><td>${cobMoeda(g.valorTotal)}</td><td>${cond}</td><td>${situacao}</td><td>${acao}</td></tr>`;
  }).join(''):'<tr><td colspan="7">Nenhum lançamento em andamento no relatório.</td></tr>';
...
function atualizarResumoCobrancaMassa(){
  const grupos=gruposSelecionadosCobrancaMassa();
  const qtd=grupos.length,parc=grupos.reduce((s,g)=>s+g.parcelas.length,0),valor=grupos.reduce((s,g)=>s+g.valorTotal,0);
  const a=document.getElementById('cobMassaQtd'),b=document.getElementById('cobMassaParcelas'),c=document.getElementById('cobMassaValor');
  if(a)a.textContent=qtd;if(b)b.textContent=parc;if(c)c.textContent=cobMoeda(valor);
}
function ordemCobrancaMassa(grupos){
  return [...grupos].sort((a,b)=>b.valorTotal-a.valorTotal).map(g=>({...g,parcelas:[...g.parcelas].sort((a,b)=>Number(a.parcela_numero||0)-Number(b.parcela_numero||0))}));
}
function abrirOrdemImpressaoCobrancaMassa(){
  const selecionados=gruposSelecionadosCobrancaMassa();
  const base=selecionados.length?selecionados:(cobrancaMassaRelatorios||[]).map(dadosGrupoCobrancaMassa).filter(g=>g.pronto);
  if(!base.length)return alert('Não há cobranças preparadas para conferir.');
  const grupos=ordemCobrancaMassa(base);
  const w=window.open('','_blank','width=1100,height=800');if(!w)return alert('Permita pop-ups para abrir a conferência.');
  const linhas=grupos.map((g,gi)=>`<section><h2>${gi+1}. ${escaparHtmlEmail(g.rel.nome||'')} <small>${cobMoeda(g.valorTotal)} • ${escaparHtmlEmail(g.rel.banco||'')}</small></h2><table><thead><tr><th>Parcela</th><th>Vencimento</th><th>Valor</th><th>Referência</th></tr></thead><tbody>${g.parcelas.map(p=>`<tr><td><b>${p.parcela_numero||1}/${p.parcela_total||g.parcelas.length}</b></td><td>${p.vencimento?new Date(p.vencimento+'T12:00:00').toLocaleDateString('pt-BR'):'—'}</td><td>${cobMoeda(p.valor)}</td><td>${escaparHtmlEmail(p.referencia||'')}</td></tr>`).join('')}</tbody></table></section>`).join('');
  w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Ordem de impressão - Boletos</title><style>body{font-family:Arial;padding:24px;color:#222}h1{margin:0 0 6px}p{color:#666}section{break-inside:avoid;margin:22px 0;padding:15px;border:1px solid #ddd;border-radius:12px}h2{font-size:18px;margin:0 0 10px}h2 small{font-size:13px;font-weight:normal;color:#666;margin-left:10px}table{width:100%;border-collapse:collapse}th,td{padding:8px;border-bottom:1px solid #eee;text-align:left}@media print{button{display:none}body{padding:0}section{border:0;border-bottom:1px solid #aaa;border-radius:0}}</style></head><body><button onclick="window.print()">🖨 Imprimir conferência</button><h1>Ordem dos boletos</h1><p>Clientes: maior valor total → menor. Parcelas do mesmo cliente permanecem juntas e em sequência.</p>${linhas}</body></html>`);w.document.close();
}
async function escolherPastaCobrancaMassa(){
  if(!window.showDirectoryPicker) return null;
  try{
    const h=await window.showDirectoryPicker({mode:'readwrite'});
    cobrancaMassaPastaHandle=h;
    const el=document.getElementById('cobMassaPasta');if(el)el.textContent=`Pasta selecionada: ${h.name}`;
    return h;
...
async function consultarPendentesBradescoSandbox(){
  ...
  if(out)out.innerHTML='⏳ Autenticando via mTLS/OAuth e consultando o recurso oficial de títulos pendentes...';if(btn)btn.disabled=true;
  try{
    ...
    let dica='';const m=String(e.message||'');if(/403|proibid|permiss/i.test(m))dica=' O Bradesco autenticou a chamada, mas o recurso pode não estar vinculado à credencial/aplicação usada.';else if(/401|token|mTLS|autentica/i.test(m))dica=' O Bradesco recusou a autenticação desta chamada: confira o vínculo da credencial, escopo e mTLS.';else if(/400|412|parâmetro|parametro|inválid|invalid/i.test(m))dica=' O endpoint foi alcançado; agora precisamos ajustar o payload conforme a validação retornada pelo próprio Bradesco Sandbox.';
...
function invalidarPreparoPilotoBB(){
  ...
  const btn=document.getElementById('btnBbEmitirPiloto');if(btn)btn.disabled=true;
}
async function previsualizarBoletoPilotoBB(){
  const body=dadosBoletoPilotoBB();const out=document.getElementById('bbPilotoResultado');
  if(bbAmbiente()!=='producao')return alert('Selecione o ambiente PRODUÇÃO para preparar a emissão piloto real.');
...
async function validarEndpointRegistroBradescoSandbox(){
  ...
  }finally{/* V229: após uma tentativa, o botão permanece bloqueado nesta tela para evitar reenvio acidental. */}
}
...
async function diagnosticarPayloadMinimoBradescoV187(){
  ...
  try{
    ...
    const rejeitados=Array.isArray(resumo.campos_rejeitados)?resumo.campos_rejeitados:[];
    const formatos=Array.isArray(resumo.erros_formato)?resumo.erros_formato:[];
    const regras=Array.isArray(resumo.regras)?resumo.regras:[];
    const resumoHtml=`<div style="margin-top:10px;padding:10px;background:#f7fff9;border:1px solid #c6e6ce;border-radius:8px;"><b>Resumo V199 — resultado principal</b><br>Mensagens únicas: ${Number(resumo.total_unicas||0)}<br><br><b>Campos não reconhecidos:</b><br>${rejeitados.length?rejeitados.map(x=>'❌ '+escaparHtmlEmail(x)).join('<br>'):'Nenhum.'}<br><br><b>Campos obrigatórios ausentes:</b><br>${ausentes.length?ausentes.map(x=>'⚠️ '+escaparHtmlEmail(x)).join('<br>'):'Nenhum entre os itens analisados.'}<br><br><b>Formato/valor inválido:</b><br>${formatos.length?formatos.map(x=>'⚠️ '+escaparHtmlEmail(x)).join('<br>'):'Nenhum reconhecido.'}<br><br><b>Outras regras:</b><br>${regras.length?regras.map(x=>escaparHtmlEmail(x)).join('<br>'):'Nenhuma regra adicional reconhecida.'}<br><br><b>Cenários internos do Sandbox ignorados:</b><br>${Number(resumo.cenarios_sandbox_ignorados||0)} mensagens de arquivos erro-*.json não foram usadas para classificar o boleto.</div>`;
    if(out){
      out.className='bb-cert-aviso ok';
      const concluida=!!d.validacao_estrutural_concluida;
      const etapa=concluida?`<div style="margin-top:10px;padding:12px;background:#eef8ff;border:1px solid #9bc9e8;border-radius:8px;"><b>✅ Validação estrutural concluída.</b><br>O HTTP 422 atual não aponta erro de campo: o payload sintético apenas não corresponde a uma massa/cenário fixo do Sandbox.<br><br><b>Próximo passo para homologação/produção:</b> usar somente os dados bancários oficiais fornecidos pelo Bradesco para o beneficiário — CPF/CNPJ do beneficiário, carteira (idProduto) e número da negociação (nuNegociacao). O sistema não tentará adivinhar esses valores.</div>`:'';
      out.innerHTML=`✅ <b>Estrutura sanitizada recebida — HTTP ${escaparHtmlEmail(String(d.http_status||''))}.</b><br>${d.codigo?`Código: ${escaparHtmlEmail(String(d.codigo))}<br>`:''}${escaparHtmlEmail(String(d.mensagem||'Validação acionada.'))}${etapa}${resumoHtml}<div style="margin-top:10px;max-height:420px;overflow:auto;background:#fff;border:1px solid #c6e6ce;border-radius:8px;padding:10px;"><pre style="margin:0;white-space:pre-wrap;word-break:break-word;font-size:12px;">${escaparHtmlEmail(texto)}</pre></div><span class="bb-cert-ajuda">🔒 V199 reconhece o HTTP 422 de cenário fixo do Sandbox quando a estrutura já não apresenta erros. Mantém vlNominalTitulo com duas casas decimais e não tenta descobrir dados bancários por tentativa. Token, Client Secret, Authorization, certificado, chave privada, credenciais e o payload enviado não são retornados ao navegador.</span>`;
    }
...
// V224 — busca inteligente de cliente para a homologação Bradesco.
// Pesquisa localmente a lista já carregada por nome, CPF ou CNPJ e preenche o endereço cadastrado.
function somenteDigitosClienteBradesco(v){return String(v||'').replace(/\D/g,'');}
function documentoClienteBradesco(c){return c?.cpf_cnpj||c?.cnpj||c?.cpf||c?.documento||'';}
function buscarClienteBradescoInteligente(termo){
  const box=document.getElementById('brClienteSugestoes'); if(!box)return;
  const q=String(termo||'').trim().toLocaleLowerCase('pt-BR'), qd=somenteDigitosClienteBradesco(termo);
  const lista=(emailClientes||[]).filter(c=>{
    const nome=String(c?.nome||'').toLocaleLowerCase('pt-BR'), doc=somenteDigitosClienteBradesco(documentoClienteBradesco(c));
    return !q || nome.includes(q) || (qd && doc.includes(qd));
  }).slice(0,20);
  if(!lista.length){box.innerHTML='<div style="padding:10px;color:#6b6680;">Nenhum cliente encontrado.</div>';box.style.display='block';return;}
  box.innerHTML=lista.map(c=>{const doc=somenteDigitosClienteBradesco(documentoClienteBradesco(c));return `<button type="button" onclick="selecionarClienteBradesco('${String(c.id).replace(/'/g,"\\'")}')" style="display:block;width:100%;text-align:left;padding:9px 11px;border:0;border-bottom:1px solid #eee;background:#fff;cursor:pointer;"><b>${escaparHtmlEmail(String(c.nome||''))}</b><br><small>${escaparHtmlEmail(doc||'Sem CPF/CNPJ cadastrado')}</small></button>`}).join('');
  box.style.display='block';
}
function selecionarClienteBradesco(id){
  const c=(emailClientes||[]).find(x=>String(x.id)===String(id)); if(!c)return;
  const set=(id,v)=>{const e=document.getElementById(id);if(e)e.value=v??''};
  set('brTesteNome',c.nome||''); set('brTesteDocumento',somenteDigitosClienteBradesco(documentoClienteBradesco(c)));
  set('brTesteCep',somenteDigitosClienteBradesco(c.cep||'')); set('brTesteLogradouro',c.endereco||c.logradouro||''); set('brTesteNumero',c.numero||''); set('brTesteComplemento',c.complemento||''); set('brTesteBairro',c.bairro||''); set('brTesteMunicipio',c.cidade||c.municipio||''); set('brTesteUf',String(c.uf||'').toUpperCase());
  const box=document.getElementById('brClienteSugestoes');if(box)box.style.display='none';
}
document.addEventListener('click',e=>{const box=document.getElementById('brClienteSugestoes'),inp=document.getElementById('brTesteNome');if(box&&inp&&!box.contains(e.target)&&e.target!==inp)box.style.display='none';});
...
function carregarProximoSeuNumeroBradesco(){ /* V227: removido — Seu Nº não é sequencial bancário. */ }
function dadosFormularioRegistroBradesco(){
  const get=id=>document.getElementById(id)?.value||'';
...
async function testarRegistroBradescoSandboxV230(){
  ...
  if(bradescoSandboxSeuNumerosTentadosV230.has(String(body.seuNumero||'').trim())) return alert('Este Seu Nº já teve uma tentativa de registro nesta tela. Use outro pedido/NF ou recarregue somente depois de analisar o retorno do Bradesco.');
  if(!body.nome)faltas.push('nome do pagador');if(![11,14].includes(body.documento.length))faltas.push('CPF/CNPJ');if(!(body.valor>0))faltas.push('valor');if(!body.vencimento)faltas.push('vencimento');if(!body.seuNumero||body.seuNumero.length>10)faltas.push('Seu Nº (NF/pedido, até 10 caracteres)');
  if(faltas.length)return alert('Complete antes do teste: '+faltas.join(', ')+'.');
  const doc=body.documento.length===11?body.documento.replace(/^(\d{3})\d+(\d{2})$/,'$1••••••$2'):body.documento.replace(/^(\d{2})\d+(\d{2})$/,'$1••••••••$2');
  const resumo=`ATENÇÃO — ESTE TESTE FARÁ UM POST REAL NO BRADESCO SANDBOX.\n\nCliente: ${body.nome}\nCPF/CNPJ: ${doc}\nSeu Nº (NF/Pedido): ${body.seuNumero}\nValor: R$ ${Number(body.valor).toFixed(2).replace('.',',')}\nVencimento: ${body.vencimento}\n\nNosso Nº (nuTitulo) NÃO será enviado; aguardaremos o retorno do Bradesco.\nNão haverá repetição automática.\n\nDeseja continuar?`;
  if(!confirm(resumo))return;
  const palavra=prompt('Confirmação final de segurança.\n\nDigite exatamente TESTAR SANDBOX para enviar esta cobrança ao endpoint de registro do Bradesco Sandbox:');
  if(String(palavra||'').trim().toUpperCase()!=='TESTAR SANDBOX')return alert('Teste cancelado. Nenhum dado foi enviado ao endpoint de registro.');
  const out=document.getElementById('bradescoRegistroRealV230Status'),btn=document.getElementById('btnRegistrarSandboxBradescoV230');
  if(out){out.className='bb-cert-aviso';out.textContent='⏳ Enviando UMA tentativa ao endpoint de registro do Bradesco Sandbox. Aguarde...';} if(btn)btn.disabled=true;
  try{
...
async function prepararHomologacaoControladaBradesco(){
  ...
  if(!seuNumero)faltas.push('Seu Nº (NF/pedido)');
  if(seuNumero.length>10)faltas.push('Seu Nº com no máximo 10 caracteres');
...
// V254 — emissão real Bradesco Produção com sanitização de texto e retentativa segura após HTTP 400; resposta persistida e HTTP 2xx copiado para Histórico com prévia congelada por hash,
...
async function emitirCobrancaProducaoBradescoV248(){
  ...
  try{
    ...
    let historicoMsg='';
    if(ok){
      // V250: a primeira emissão real aceita pelo Bradesco também precisa virar um
