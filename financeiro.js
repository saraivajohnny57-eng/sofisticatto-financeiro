...
async function abrirEmissaoBoletoRelatorio(relatorioId){
  ...
  if(btnEmitirModal)btnEmitirModal.textContent=codigoBancoCobranca(item.banco)==="bb"?"💳 Salvar parcelas e emitir BB":"💳 Emitir boleto(s) no Bradesco";
  document.getElementById("bolValor").value=valorParaInput(boletoCobrancaAtual?.valor_total_grupo??item.valor_total??item.valor??0);
...
async function salvarPreparacaoBoletoRelatorioV175(){
  ...
  // V260: uma preparação cancelada não pode gerar novos registros duplicados.
  // Antes de criar o grupo, removemos somente preparações LOCAIS ainda pendentes deste mesmo relatório/banco.
  // Registros abertos/pagos/vencidos/cancelados nunca são removidos aqui.
  try{
    await banco.from("cobrancas_bancarias")
      .delete()
      .eq("relatorio_id",boletoRelatorioAtual.id)
      .eq("banco",codigoBancoCobranca(boletoRelatorioAtual.banco))
