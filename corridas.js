...
async function lancarCorrida(){
  ...
  const r=await banco.from('corridas').insert([{entregador_id:ent.id,entregador_nome:ent.nome,categoria:ent.categoria||null,telefone:ent.telefone||null,retirada,destino,observacao:observacao||null,valor,data_corrida,status:'aberta',criado_por:usuarioLogado?.login||null}]).select('numero_corrida').single();
  ...
  ['corridaRetirada','corridaDestino','corridaObservacao','corridaValor'].forEach(id=>document.getElementById(id).value='');
...
async function abrirFechamentoCorridas(id){
  ...
  document.getElementById('modalFechamentoTabela').innerHTML=arr.map(x=>`<tr><td>${codigoCorrida(x.numero_corrida)}</td><td>${fmtDataCorridas(x.data_corrida)}</td><td>${escCorridas(x.retirada)}</td><td>${escCorridas(x.destino)}</td><td>${escCorridas(x.observacao||'—')}</td><td>${fmtMoedaCorridas(x.valor)}</td></tr>`).join('');
