...
module.exports=async function handler(req,res){
  ...
  const cnpjRem=somenteNumeros(e.cnpj_remetente||"05451985000195");
  const cepRem=somenteNumeros(e.cep_remetente||"74550470");
  const razaoRem=String(e.razao_remetente||"SOFISTICATTO COSMÉTICOS").trim();
  ...
  if(cnpjRem.length!==14)faltam.push("CNPJ do remetente/origem");
  if(cepRem.length!==8)faltam.push("CEP do remetente/origem");
  ...
  const tipoFrete=String(e.tipo_frete||"CIF").toUpperCase()==="FOB"?"FOB":"CIF";
  const cnpjTomador=tipoFrete==="FOB"&&doc.length===14?doc:cnpjRem;
  ...
  const payload={
    ...
    // Remetente/origem — V93
    cepRem:cepRem,
    cnpjRem:cnpjRem,

    // Aliases explícitos. A API Alfa ignora campos que não utiliza,
    // mas versões/parametrizações distintas podem reconhecer um destes nomes.
    remCnpj:cnpjRem,
    cnpjRemetente:cnpjRem,
    remetenteCnpj:cnpjRem,
    remCep:cepRem,
    remetenteCep:cepRem,
    remRazao:razaoRem,

    // Tomador/pagador
    tipoPagador:tipoFrete==="FOB"?2:1,
    tomCnpj:cnpjTomador,
    cnpjTomador:cnpjTomador,
    cnpjPagador:cnpjTomador,
    ...
    try{
      await supabaseRest("integracao_logs",{method:"POST",body:{
        ...
        resposta_resumida:{
          ...
          prazo,
          m3,
          cnpj_destinatario_enviado:doc,
          cnpj_remetente_enviado:cnpjRem,
          cep_remetente_enviado:cepRem,
          cnpj_tomador_enviado:cnpjTomador,
