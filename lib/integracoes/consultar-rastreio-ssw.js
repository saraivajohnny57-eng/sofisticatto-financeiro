...
function eventoTrackingSSW(d){
  const lista=Array.isArray(d?.documento?.tracking)?d.documento.tracking:[];
  if(!lista.length)return null;
  const ordenada=lista.slice().sort((a,b)=>{
    const da=Date.parse(a?.data_hora_efetiva||a?.data_hora||'')||0;
    const db=Date.parse(b?.data_hora_efetiva||b?.data_hora||'')||0;
    return db-da;
  });
  const o=ordenada[0]||{};
  const titulo=String(o.ocorrencia||o.status||'Ocorrência SSW').trim();
  const detalhe=String(o.descricao||'').trim();
  const local=[o.cidade,o.filial].filter(Boolean).join(' • ');
  return {desc:[titulo,detalhe].filter(Boolean).join(' — '),data:o.data_hora_efetiva||o.data_hora||null,compl:local||null,raw:o};
}
...
module.exports=async function(req,res){
  ...
  try{
    ...
    const ev=eventoTrackingSSW(dados)||eventoMaisUtil(dados);const desc=String(ev.desc||'Rastreamento SSW consultado');const status=statusPorTexto(JSON.stringify(dados)+' '+desc);
    let quando=new Date().toISOString();if(ev.data){const t=Date.parse(ev.data);if(Number.isFinite(t))quando=new Date(t).toISOString();}
    const patch={status,status_api:desc,ultima_ocorrencia:[desc,ev.compl].filter(Boolean).join(' — '),ultima_ocorrencia_em:quando,consulta_api:dados,metodo_consulta:metodo,consultado_api_em:new Date().toISOString(),sincronizacao_erro:null,atualizado_em:new Date().toISOString(),atualizado_por:'api_ssw_tracking_v49'};
