...
module.exports=async function(req,res){
  ...
  try{
    ...
    const integ=await buscarIntegracao(nome);if(!integ?.convite_id)return json(res,422,{ok:false,erro:`Integração SSW não localizada para ${nome}.`});
    ...
    if(!dados){
      ...
      if(!dados && !senha)return json(res,422,{ok:false,codigo:'SSW_SENHA_RASTREIO_NAO_CADASTRADA',erro:`O SSW não liberou a consulta sem senha para ${nome}. Cadastre a senha de rastreio/pagador criada na opção 383.`,resposta:ultimoErro?.resposta||null});
