...
module.exports=async function(req,res){
  ...
  try{
    ...
    if(rast.coleta_agendamento_id && ['em_transito','na_filial','saiu_entrega','entregue','ocorrencia'].includes(status)){
      ...
      try{
        const rows=await supabaseRest('coleta_agendamentos',{query:`?select=id,status,status_api,coletado_em&id=eq.${encodeURIComponent(rast.coleta_agendamento_id)}&limit=1`});
        coletaAtual=Array.isArray(rows)?rows[0]:null;
      }catch{}
      const jaColetada=/coletad/i.test(String(coletaAtual?.status||''));
      ...
      // Registra a transição somente uma vez. Consultas posteriores atualizam
      // a ocorrência/status_api, mas não criam novo "Solicitada → Coletada".
      if(!jaColetada){
