...
module.exports=async function(req,res){
  ...
  try{
    ...
    if(tipoFrete==='CIF'&&cnpjPagador!==cnpjRem)return json(res,422,{ok:false,erro:'Na cotação CIF, o CNPJ pagador deve ser o CNPJ do remetente.'});
    if(tipoFrete==='FOB'&&cnpjPagador===cnpjRem)return json(res,422,{ok:false,erro:'Na cotação FOB, o CNPJ pagador deve ser diferente do CNPJ do remetente.'});
    ...
    // V132: no FOB normal, o cnpjRemetente não deve ser enviado ao método SOAP.
    // Enviar a tag vazia ainda deixa o parâmetro presente e algumas bases SSW
    // (ACCERT/TG) continuam interpretando a chamada pela regra de remetente/CIF.
    // CIF mantém o remetente; FOB usa o destinatário como pagador e OMITE a tag.
    const payloadSoap={
      ...
      fatorMultiplicador,
      cnpjRemetente:tipoFrete==='CIF'?cnpjRem:''
    };
    const estrategiaFob=tipoFrete==='FOB'?'fob-normal-omitindo-tag-remetente':'cif-remetente-informado';
