...
module.exports=async function(req,res){
  ...
  try{
    ...
    let dados;
    let estrategiaFob='padrao';
    try{
      dados=await chamarSoap(payloadSoap);
    }catch(e){
      // V131: algumas bases SSW de ACCERT/TG interpretam o CNPJ do remetente
      // como FOB Dirigido. Para FOB normal, se a própria SSW acusar conflito
      // CIF/pagador x remetente, repetimos sem vincular o remetente.
      const msg=String(e?.message||'');
      if(tipoFrete==='FOB'&&/pagador.*diferente.*remetente.*frete\s*CIF/i.test(msg)){
        estrategiaFob='fob-normal-sem-vinculo-remetente';
        dados=await chamarSoap({...payloadSoap,cnpjRemetente:''});
      }else throw e;
    }
    return json(res,200,{
      ok:true,
