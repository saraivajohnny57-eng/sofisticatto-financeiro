...
function soapEnvelope(ns,p){
  // V135: mantém ciffob explícito, mas volta a priorizar o padrão SOAP que já funcionava no portal.
  // O campo ciffob é obrigatório para o SSW distinguir CIF de FOB.
...
async function chamarSoap(payload){
  ...
  // V135: prioriza exatamente os namespaces/ações que já eram usados quando a cotação funcionava.
  // As variantes sswinfbr ficam apenas como fallback.
  const tentativas=[
    {ns:'urn:sswCotacao',action:'urn:sswCotacao#cotar'},
    {ns:'urn:sswCotacao',action:'cotar'},
    {ns:'http://ssw.inf.br/ws/sswCotacao',action:'cotar'},
    {ns:'urn:sswinfbr.sswCotacao',action:'urn:sswinfbr.sswCotacao#cotar'},
    {ns:'urn:sswinfbr.sswCotacao',action:'cotar'}
  ];
  let ultimo;
  const diagnostico=[];
  for(const t of tentativas){
    try{
      ...
      const fault=tag(txt,'faultstring');if(fault)throw Object.assign(new Error(fault),{httpStatus:422,resposta:txt.slice(0,2000)});
      ...
      if(Number.isFinite(erro)&&erro<0){
        diagnostico.push({ns:t.ns,action:t.action,erro,mensagem,retorno:retorno.slice(0,900)});
        ultimo=Object.assign(new Error(mensagem||`SSW retornou erro ${erro}`),{httpStatus:422,resposta:retorno});
        continue;
      }
      if(!totalFrete && Number.isFinite(erro)&&erro!==0&&erro!==1){
        diagnostico.push({ns:t.ns,action:t.action,erro,mensagem,retorno:retorno.slice(0,900)});
        ultimo=Object.assign(new Error(mensagem||`SSW retornou erro ${erro}`),{httpStatus:422,resposta:retorno});
        continue;
      }
      // Importante: HTTP 200 + erro=0 sem valor NÃO é uma cotação válida.
      // Na V134 isso encerrava na primeira tentativa e escondia as demais combinações SOAP.
      if(!totalFrete){
        diagnostico.push({ns:t.ns,action:t.action,erro:Number.isFinite(erro)?erro:null,mensagem:mensagem||'Sem valor de frete',retorno:retorno.slice(0,900)});
        ultimo=Object.assign(new Error(mensagem||'SSW respondeu sem valor de frete nesta variante SOAP.'),{httpStatus:422,resposta:retorno});
        continue;
      }
      return {erro:Number.isFinite(erro)?erro:null,mensagem,valor:totalFrete,prazo,xml:retorno,detalhes:{pesoCalculo:numero(tag(retorno,'pesoCalculo')),tabCalculo:tag(retorno,'tabCalculo'),soap:{ns:t.ns,action:t.action}}};
    }catch(e){
      ultimo=e;
      diagnostico.push({ns:t.ns,action:t.action,erro:null,mensagem:e.message,resposta:String(e.resposta||'').slice(0,900)});
    }
  }
  const err=ultimo||new Error('Não foi possível chamar o WebService de Cotação SSW.');
  err.httpStatus=err.httpStatus||422;
  err.resposta=JSON.stringify({tentativas:diagnostico.slice(-5)},null,2);
  throw err;
}
