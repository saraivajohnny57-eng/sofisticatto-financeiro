...
function soapEnvelope(ns,p){
  // V136: assinatura completa usada pelo sswCotacaoColeta, que possui ciffob.
  return `<?xml version="1.0" encoding="UTF-8"?>\n<soapenv:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:urn="${ns}"><soapenv:Header/><soapenv:Body><urn:cotar soapenv:encodingStyle="http://schemas.xmlsoap.org/soap/encoding/"><dominio xsi:type="xsd:string">${xmlEsc(p.dominio)}</dominio><login xsi:type="xsd:string">${xmlEsc(p.login)}</login><senha xsi:type="xsd:string">${xmlEsc(p.senha)}</senha><cnpjPagador xsi:type="xsd:string">${xmlEsc(p.cnpjPagador)}</cnpjPagador><cepOrigem xsi:type="xsd:integer">${xmlEsc(p.cepOrigem)}</cepOrigem><cepDestino xsi:type="xsd:integer">${xmlEsc(p.cepDestino)}</cepDestino><valorNF xsi:type="xsd:decimal">${xmlEsc(p.valorNF)}</valorNF><quantidade xsi:type="xsd:integer">${xmlEsc(p.quantidade)}</quantidade><peso xsi:type="xsd:decimal">${xmlEsc(p.peso)}</peso><volume xsi:type="xsd:decimal">${xmlEsc(p.volume)}</volume><mercadoria xsi:type="xsd:integer">${xmlEsc(p.mercadoria)}</mercadoria><ciffob xsi:type="xsd:string">${xmlEsc(p.ciffob)}</ciffob><cnpjRemetente xsi:type="xsd:string">${xmlEsc(p.cnpjRemetente)}</cnpjRemetente><cnpjDestinatario xsi:type="xsd:string">${xmlEsc(p.cnpjDestinatario)}</cnpjDestinatario><observacao xsi:type="xsd:string">${xmlEsc(p.observacao||'')}</observacao><trt xsi:type="xsd:string">${xmlEsc(p.trt||'N')}</trt><coletar xsi:type="xsd:string">${xmlEsc(p.coletar)}</coletar><entDificil xsi:type="xsd:string">${xmlEsc(p.entDificil)}</entDificil><destContribuinte xsi:type="xsd:string">${xmlEsc(p.destContribuinte)}</destContribuinte><qtdePares xsi:type="xsd:integer">${xmlEsc(p.qtdePares)}</qtdePares><altura xsi:type="xsd:decimal">${xmlEsc(p.altura)}</altura><comprimento xsi:type="xsd:decimal">${xmlEsc(p.comprimento)}</comprimento><largura xsi:type="xsd:decimal">${xmlEsc(p.largura)}</largura><fatorMultiplicador xsi:type="xsd:integer">${xmlEsc(p.fatorMultiplicador)}</fatorMultiplicador></urn:cotar></soapenv:Body></soapenv:Envelope>`;
}
function soapEnvelopeLegado(ns,p){
  // Assinatura antiga do sswCotacao: sem ciffob/observacao/trt no meio dos parâmetros.
...
function interpretarRetornoSSW(txt){
  const fault=tag(txt,'faultstring');
  if(fault)throw Object.assign(new Error(fault),{httpStatus:422,resposta:String(txt||'').slice(0,2000)});
  ...
  return {retorno,erro,mensagem,totalFrete,prazo};
}
async function chamarSoap(payload){
  // V136: o endpoint com a assinatura COMPLETA (inclui ciffob) é sswCotacaoColeta.
  // O endpoint sswCotacao é mantido apenas como fallback legado, principalmente para CIF.
  const tentativas=[
    {endpoint:'https://ssw.inf.br/ws/sswCotacaoColeta/index.php',ns:'urn:sswinfbr.sswCotacao',action:'cotar',modo:'completo'},
    {endpoint:'https://ssw.inf.br/ws/sswCotacaoColeta/index.php',ns:'urn:sswinfbr.sswCotacao',action:'urn:sswinfbr.sswCotacao#cotar',modo:'completo'},
    {endpoint:'https://ssw.inf.br/ws/sswCotacaoColeta/index.php',ns:'urn:sswCotacao',action:'cotar',modo:'completo'},
    {endpoint:'https://ssw.inf.br/ws/sswCotacao/index.php',ns:'urn:sswCotacao',action:'urn:sswCotacao#cotar',modo:'legado'},
    {endpoint:'https://ssw.inf.br/ws/sswCotacao/index.php',ns:'urn:sswCotacao',action:'cotar',modo:'legado'}
  ];
  let ultimo; const diagnostico=[];
  for(const t of tentativas){
    // Para FOB, o legado não sabe distinguir a modalidade e já comprovadamente devolve conflito de CIF.
    if(payload.ciffob==='F' && t.modo==='legado')continue;
    try{
      const body=t.modo==='completo'?soapEnvelope(t.ns,payload):soapEnvelopeLegado(t.ns,payload);
      const r=await fetch(t.endpoint,{method:'POST',headers:{'Content-Type':'text/xml; charset=utf-8','SOAPAction':`"${t.action}"`,Accept:'text/xml'},body});
      const txt=await r.text();
      if(!r.ok)throw Object.assign(new Error(`SSW Cotação HTTP ${r.status}`),{httpStatus:r.status,resposta:txt.slice(0,2000)});
      const d=interpretarRetornoSSW(txt);
      if(Number.isFinite(d.erro)&&d.erro<0){
        diagnostico.push({endpoint:t.endpoint.split('/ws/')[1],modo:t.modo,ns:t.ns,action:t.action,erro:d.erro,mensagem:d.mensagem,retorno:d.retorno.slice(0,1000)});
        ultimo=Object.assign(new Error(d.mensagem||`SSW retornou erro ${d.erro}`),{httpStatus:422,resposta:d.retorno});
        continue;
      }
      if(!d.totalFrete){
        diagnostico.push({endpoint:t.endpoint.split('/ws/')[1],modo:t.modo,ns:t.ns,action:t.action,erro:Number.isFinite(d.erro)?d.erro:null,mensagem:d.mensagem||'Sem valor de frete',retorno:d.retorno.slice(0,1000)});
        ultimo=Object.assign(new Error(d.mensagem||'SSW respondeu sem valor de frete nesta variante SOAP.'),{httpStatus:422,resposta:d.retorno});
        continue;
      }
      return {erro:Number.isFinite(d.erro)?d.erro:null,mensagem:d.mensagem,valor:d.totalFrete,prazo:d.prazo,xml:d.retorno,detalhes:{pesoCalculo:numero(tag(d.retorno,'pesoCalculo')),tabCalculo:tag(d.retorno,'tabCalculo'),soap:{endpoint:t.endpoint.split('/ws/')[1],modo:t.modo,ns:t.ns,action:t.action}}};
    }catch(e){
      ultimo=e;
      diagnostico.push({endpoint:t.endpoint.split('/ws/')[1],modo:t.modo,ns:t.ns,action:t.action,erro:null,mensagem:e.message,resposta:String(e.resposta||'').slice(0,1000)});
    }
  }
  const err=ultimo||new Error('Não foi possível chamar o WebService de Cotação SSW.');
  err.httpStatus=err.httpStatus||422;
  err.resposta=JSON.stringify({tipo:payload.ciffob==='F'?'FOB':'CIF',tentativas:diagnostico},null,2);
  throw err;
}
module.exports=async function(req,res){
  ...
  try{
    ...
    const tipoFrete=String(b.tipo_frete||'CIF').trim().toUpperCase()==='FOB'?'FOB':'CIF';
    const cnpjDest=String(b.cnpj_destinatario||'').replace(/\D/g,'');
    const cnpjRem=String(b.cnpj_remetente||'05451985000195').replace(/\D/g,'');

    // V133: o backend é a fonte de verdade para quem paga o frete.
    // CIF = remetente/Sofisticatto paga. FOB = destinatário/cliente paga.
    // Não confiamos em cnpj_pagador vindo da tela para evitar mistura entre os dois cartões.
    const cnpjPagador=tipoFrete==='CIF'?cnpjRem:cnpjDest;

    if(cnpjRem.length!==14)return json(res,422,{ok:false,erro:'CNPJ do remetente inválido para cotação SSW.'});
    if(cnpjDest.length!==14)return json(res,422,{ok:false,erro:`Na cotação ${tipoFrete}, o destinatário precisa possuir CNPJ de 14 dígitos para o WebService SSW.`});
    ...
    // V133: remetente e destinatário são sempre os mesmos participantes da operação.
    // O que muda entre CIF e FOB é SOMENTE o pagador do frete.
    const ciffob=tipoFrete==='FOB'?'F':'C';
    const payloadSoap={
      ...
      observacao:`Cotacao portal Sofisticatto - ${tipoFrete}`,
      trt:'N',
    ...
    const estrategiaFob=tipoFrete==='FOB'?'fob-ciffob-F-pagador-destinatario':'cif-ciffob-C-pagador-remetente';
    const dados=await chamarSoap(payloadSoap);
    return json(res,200,{
      ok:true,
      transportadora:tr.nome,
      valor:dados.valor,
