...
module.exports=async function handler(req,res){
  ...
  try{
    ...
    const lista=await supabaseRest("coleta_agendamentos",{
      query:`?select=id,cliente_nome,transportadora_id,codigo_coleta,protocolo_cotacao,status,status_api,data_programada,sincronizacao_erro&transportadora_id=in.(${ids})&status=in.(${status})&order=created_at.asc&limit=100`
    });
    ...
    const consultaveis=(lista||[]).filter(a=>String(a.codigo_coleta||"").replace(/\D/g,""));
    const semCodigo=(lista||[]).filter(a=>!String(a.codigo_coleta||"").replace(/\D/g,""));
    ...
    for(const agendamento of semCodigo){
      resultados.push({
        ...
        ignorado:true,
        motivo:"A Rodonaves ainda não retornou um código de coleta consultável pela API."
      });
    }
    ...
    if(!consultaveis.length){
      ...
      return json(res,200,{
        ok:true,total,consultadas:0,alteradas:0,ignoradas,erros:0,
        tempo_ms:Date.now()-inicio,resultados
      });
    }
    ...
    return json(res,200,{
      ok:true,total,consultadas,alteradas,ignoradas,erros,
