...
async function conviteBraspress(){const rows=await supabaseRest('transportadora_integracoes',{query:'?select=convite_id,transportadora_nome&transportadora_nome=ilike.*Braspress*&limit=1'});return rows?.[0]?.convite_id||null;}

module.exports=async function(req,res){
  if(req.method!=='GET')return json(res,405,{ok:false,erro:'Método não permitido.'});
  if(!exigirAdmin(req,res))return;
  const registroId=txt(req.query.registro_id), nf=soDigitos(req.query.nfe||req.query.nf);
  if(!nf)return json(res,400,{ok:false,erro:'Informe o número da NF para rastrear na Braspress.'});
  try{
    const conviteId=await conviteBraspress();if(!conviteId)throw new Error('Integração Braspress não localizada. Salve novamente o cadastro rápido.');
    const c=await credenciais(conviteId,'producao');const cnpj=soDigitos(c.braspress_cnpj);if(!cnpj)throw new Error('CNPJ do remetente Braspress não configurado.');
