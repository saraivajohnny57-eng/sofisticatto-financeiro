...
function extrairEventos(d){
  ...
  return arr.map(x=>{
    ...
    return {
      titulo:titulo||descricao||'Ocorrência Braspress',
      descricao,
      data:dataVal(valorCampo(x,['dataHora','dataOcorrencia','dataEvento','data','dataStatus','dataMovimento','data_hora'])),
      local:localEvento(x),
      unidade:txt(valorCampo(x,['unidade','filial','unidadeBraspress','nomeFilial','terminal'])),
      destino:txt(valorCampo(x,['destino','unidadeDestino','filialDestino','cidadeDestino'])),
      codigo:txt(valorCampo(x,['codigo','codigoOcorrencia','codOcorrencia','codigoEvento'])),
      recebedor:txt(valorCampo(x,['recebedor','nomeRecebedor','recebidoPor'])),
      documentoRecebedor:txt(valorCampo(x,['documentoRecebedor','cpfRecebedor','rgRecebedor'])),
      comprovante:txt(valorCampo(x,['comprovante','urlComprovante','linkComprovante','imagemComprovante']))
    };
  }).filter(x=>x.descricao||x.titulo||x.data);
}
...
module.exports=async function(req,res){
  ...
  try{
    ...
    const origem=txt(deep(d,['origem','cidadeOrigem','filialOrigem']));
    const destino=txt(deep(d,['destino','cidadeDestino','filialDestino']));
    const dataColeta=dataVal(deep(d,['dataColeta','coleta','dataEmissao','dataPostagem']));
    const recebedor=txt(deep(d,['recebedor','nomeRecebedor','recebidoPor']));
    const agora=new Date().toISOString();
    if(registroId){const patch={status,status_api:statusBruto,numero_cte:cte||null,previsao_entrega:previsao?previsao.slice(0,10):null,ultima_ocorrencia:ultimo?.descricao||statusBruto,ultima_ocorrencia_em:ultimo?.data||agora,consulta_api:{retorno:d,eventos,fonte:'Braspress Tracking V3',nf,origem,destino,dataColeta,recebedor,previsaoEntrega:previsao?previsao.slice(0,10):null,numeroCte:cte||null},metodo_consulta:'Braspress API V3 • NF',consultado_api_em:agora,sincronizacao_erro:null,atualizado_em:agora,atualizado_por:'api_braspress_v215'};if(status==='entregue')patch.finalizado_em=ultimo?.data||agora;await supabaseRest('logistica_rastreamentos',{method:'PATCH',query:`?id=eq.${encodeURIComponent(registroId)}`,body:patch});}
    return json(res,200,{ok:true,status,statusBruto,previsaoEntrega:previsao?previsao.slice(0,10):null,numeroCte:cte||null,origem,destino,dataColeta,recebedor,ultimaOcorrencia:ultimo?.descricao||statusBruto,ultimaOcorrenciaEm:ultimo?.data||null,eventos,dados:d,metodoConsulta:'Braspress API V3 • NF'});
