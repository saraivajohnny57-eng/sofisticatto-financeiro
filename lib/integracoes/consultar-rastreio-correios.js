...
module.exports=async(req,res)=>{if(req.method!=='GET')return json(res,405,{ok:false,erro:'Método não permitido.'});if(!exigirAdmin(req,res))return;try{
  ...
  const d=await rastrear(codigo);const n=normalizar(d);const agora=new Date().toISOString();if(registroId){const patch={protocolo_rastreio:codigo,status:n.status,status_api:n.desc,ultima_ocorrencia:n.desc,ultima_ocorrencia_em:agora,consulta_api:d,metodo_consulta:'Correios API Rastro',consultado_api_em:agora,atualizado_em:agora,atualizado_por:'api_correios_v45'};if(n.status==='entregue')patch.finalizado_em=agora;await supabaseRest('logistica_rastreamentos',{method:'PATCH',query:`?id=eq.${encodeURIComponent(registroId)}`,body:patch});}return json(res,200,{ok:true,codigoObjeto:codigo,codigoDescobertoAutomaticamente:Boolean(auto?.codigo),status:n.status,statusBruto:n.desc,dados:d});
