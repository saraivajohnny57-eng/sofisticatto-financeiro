...
function normalizar(d){
  ...
  const eventos=lista.map(ev=>({
    ...
    local:[
      ev?.unidade?.endereco?.cidade||ev?.unidade?.nome||ev?.cidade,
      ev?.unidade?.endereco?.uf||ev?.uf
    ].filter(Boolean).join(' / '),
  ...
  let status='aguardando_postagem';
  let coletado=false;

  if(/\bentregue\b|entrega realizada|objeto entregue|recebido pelo destinatario/.test(t)||cod==='BDE'){
    status='entregue'; coletado=true;
  }else if(/saiu.*para.*entrega|saida.*para.*entrega|carteiro saiu|em rota de entrega/.test(t)){
    status='saiu_entrega'; coletado=true;
  }else if(/objeto em transferencia|em transferencia|encaminhad|transferencia|objeto em transito/.test(t)){
    status='em_transito'; coletado=true;
  }else if(/chegou.*unidade|objeto recebido.*unidade|recebido.*unidade|unidade de tratamento|unidade de distribuicao/.test(t)){
    status='na_filial'; coletado=true;
  }else if(/objeto postado|postado apos|postado\b|recebido pelos correios/.test(t)){
    status='postado'; coletado=true;
  }else if(/etiqueta emitida|aguardando postagem|pre[- ]?postagem|prepostad|pre[- ]?atendid/.test(t)){
    status='aguardando_postagem'; coletado=false;
  }else{
    // Sem evidência de postagem, não presumimos que o objeto foi coletado.
    status='aguardando_postagem'; coletado=false;
  }

  return {obj,ev,eventos,status,desc,coletado};
}
...
module.exports=async(req,res)=>{if(req.method!=='GET')return json(res,405,{ok:false,erro:'Método não permitido.'});if(!exigirAdmin(req,res))return;try{
  ...
  if(registroId){
    ...
    if(n.status==='entregue')patch.finalizado_em=agora;
