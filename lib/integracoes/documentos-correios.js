...
async function emitirDaceOficial(id){
  const url='https://api.correios.com.br/prepostagem/v1/prepostagens/dce/dace/impressao';

  // A API de Pré-postagem evoluiu de versão; aceitamos as formas de corpo vistas
  // nos schemas da API sem transformar o documento localmente.
  const corpos=[
    {idsPrePostagem:[id]},
    {idPrePostagem:id},
    {ids:[id]},
  ...
  for(const corpo of corpos){
    const r=await correiosRaw(url,{
      method:'POST',
      headers:{'Content-Type':'application/json','Accept':'application/pdf, application/json, text/html, */*'},
    ...
    if(r.ok){
      const doc=documentoResposta(buf,ct);
      if(doc) return doc;

      const texto=buf.toString('utf8');
      let d={};try{d=JSON.parse(texto)}catch{d={texto}}
      ultimo={status:r.status,resposta:d};
      // Algumas versões podem devolver recibo/processamento; deixa a próxima
      // forma de corpo tentar antes de falhar.
      continue;
    }
    ...
    // 400 geralmente é schema de body incompatível: testa o próximo formato.
    if(r.status===400) continue;

    throw Object.assign(
      new Error(d.mensagem||d.message||d.erro||d.msgs?.[0]||texto||`Correios HTTP ${r.status}`),
      {httpStatus:r.status,resposta:d}
    );
  }

  const resposta=ultimo?.resposta||{};
  const msg=resposta.mensagem||resposta.message||resposta.erro||resposta.msgs?.[0]||
    'Os Correios não retornaram a DACE para esta pré-postagem.';
  throw Object.assign(new Error(msg),{httpStatus:ultimo?.status||422,resposta});
}
...
module.exports=async function handler(req,res){
  try{
    ...
    if(acao==='dace'){
      const d=await emitirDaceOficial(id);
      res.setHeader('Content-Type',d.contentType);
      const ext=/pdf/i.test(d.contentType)?'pdf':'html';
      res.setHeader('Content-Disposition',`inline; filename="DACE-Correios-${id}.${ext}"`);
      return res.status(200).send(d.buffer);
    }
