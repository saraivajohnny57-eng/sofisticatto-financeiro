...
async function converterRotuloL42(buffer){
  ...
  const mm=72/25.4, alvoW=100*mm, alvoH=150*mm;
  for(const pg of paginas){
    const {width,height}=pg.getSize();
    const cropW=Math.min(width,alvoW), cropH=Math.min(height,alvoH);
    const emb=await destino.embedPage(pg,{left:0,bottom:Math.max(0,height-cropH),right:cropW,top:height});
    const out=destino.addPage([alvoW,alvoH]);
    const escala=Math.min(alvoW/cropW,alvoH/cropH);
    const w=cropW*escala,h=cropH*escala;
    out.drawPage(emb,{x:(alvoW-w)/2,y:(alvoH-h)/2,width:w,height:h});
  }
