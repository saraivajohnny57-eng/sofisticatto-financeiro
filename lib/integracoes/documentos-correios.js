...
async function emitirDaceOficial(id){
  // V82: usa somente o recurso oficial documentado pela API de Pré-postagem.
  // O endpoint /dce/dace/impressao usado anteriormente não é o recurso público
  // documentado para emissão da declaração e estava retornando HTTP 400.
  const r=await correiosRaw(
    `https://api.correios.com.br/prepostagem/v1/prepostagens/declaracaoconteudo/${encodeURIComponent(id)}`,
    {method:'GET',headers:{Accept:'text/html, application/pdf, */*'}}
  );
