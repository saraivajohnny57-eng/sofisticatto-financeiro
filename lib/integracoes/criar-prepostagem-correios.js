...
module.exports=async function handler(req,res){
  try{
    ...
    // V125 — transporte aéreo Correios:
    ...
    // V125: envie o campo SEMPRE de forma explícita. Lista vazia significa que
    // não foi solicitado qualquer serviço adicional, inclusive o 095.
    base.listaServicoAdicional=adicionaisSeguros;
    const baseFinal=base;
    let payload;
    if(chave.length===44){
      payload={...baseFinal,tipoDocumento:'NF',numeroNotaFiscal:nf||undefined,chaveNFe:chave};
    }else{
      payload={...baseFinal,tipoDocumento:'DC',emiteDCe:'S',itensDeclaracaoConteudo:[{conteudo:mercadoria,descricao:mercadoria,quantidade:1,valor,pesoLiquidoGrama:peso}]};
    }
    ...
    console.log('[CORREIOS AEREO V125]',{id,codigoObjeto,...diagnosticoAereo});
