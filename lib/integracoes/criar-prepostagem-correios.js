...
module.exports=async function handler(req,res){
  try{
    ...
    // V126 — transporte aéreo Correios:
    ...
    // V126: 095 somente para conteúdo com restrição ANAC. Quando a mercadoria
    // pode viajar por avião, omitimos completamente o campo para não criar qualquer
    // ambiguidade na pré-postagem dos Correios.
    if(adicionaisSeguros.length) base.listaServicoAdicional=adicionaisSeguros;
    const baseFinal=base;
    let payload;
    // Desde 15/09/2025 os Correios exigem itensDeclaracaoConteudo também quando
    // há NF-e. Enviamos a descrição real da mercadoria nos dois fluxos.
    const itemConteudo={conteudo:mercadoria,descricao:mercadoria,quantidade:1,valor,pesoLiquidoGrama:peso};
    if(chave.length===44){
      payload={...baseFinal,tipoDocumento:'NF',numeroNotaFiscal:nf||undefined,chaveNFe:chave,itensDeclaracaoConteudo:[itemConteudo]};
    }else{
      payload={...baseFinal,tipoDocumento:'DC',emiteDCe:'S',itensDeclaracaoConteudo:[itemConteudo]};
    }
    ...
    const diagnosticoAereo={
      ...
      exigirPossibilidadeAerea:b.exigirPossibilidadeAerea===true,
      observacaoModal:'O adicional 095 controla restrição ANAC; a rota/modal operacional é definida pelo serviço e pela malha dos Correios.',
      coerente:restricaoAereaConfirmada?enviadoTem095:!enviadoTem095
    };
    console.log('[CORREIOS AEREO V126]',{id,codigoObjeto,...diagnosticoAereo});
    return res.status(200).json({ok:true,idPrePostagem:id,codigoObjeto,codigoServico:codigo,servico:(c.servicosDetalhes||[]).find(x=>String(x.codigo)===codigo)?.descricao||codigo,cepOriginal,cepUtilizado:destinatario.endereco.cep,cepFallback,diagnosticoAereo,raw:d});
