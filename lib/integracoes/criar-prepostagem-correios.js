...
module.exports=async function handler(req,res){
  try{
    ...
    // Proteção adicional: mesmo que algum cadastro/tela antiga envie serviços adicionais,
    // o código 095 não será repassado automaticamente. Para mercadoria realmente restrita,
    // será necessário implementar uma autorização explícita no portal.
    const adicionaisEntrada=Array.isArray(b.listaServicoAdicional)?b.listaServicoAdicional:[];
    const adicionaisSeguros=adicionaisEntrada.filter(x=>dig(x?.codigoServicoAdicional ?? x)!=='095');
