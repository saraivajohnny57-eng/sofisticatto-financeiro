...
async function marcarColetaSSWComoRealizada(item,descricao,quando){
  ...
  try{
    const rows=await supabaseRest('coleta_agendamentos',{query:`?select=id,status,status_api,coletado_em&id=eq.${encodeURIComponent(item.coleta_agendamento_id)}&limit=1`});
  ...
  await supabaseRest('coleta_agendamentos',{
    ...
    body:{
      status:'coletado',
      status_api:descricao||coletaAtual?.status_api||'coletado',
      coletado_em:coletaAtual?.coletado_em||quando||agora,
