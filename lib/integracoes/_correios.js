...
async function configuracaoEfetiva(){
  ...
  if(servicosAg.length){
    ...
    modoServicos='CONTRATO_AG';
  }else{
    // Fallback para contratos que não usam a nomenclatura AG na API Meu Contrato.
    // Mantém compatibilidade com a V66 e garante tentativa de PAC e SEDEX.
    servicos=[...new Set([...(c.servicos||[]),...autoServicos,'03298','03220'])];
    servicosDetalhes=autoDetalhes;
    modoServicos='FALLBACK';
  }
...
async function cotar({cepDestino,pesoKg,comprimento=20,largura=20,altura=20,servicos}){
 ...
 // Consulta os serviços do contrato em paralelo. A versão anterior fazia uma
 // modalidade por vez (preço e depois prazo), o que deixava a tela lenta.
 const unicos=[...new Set(lista)];
 const consultados=await Promise.all(unicos.map(coProduto=>consultarServico(coProduto)));
 resultados.push(...consultados);
 const validos=resultados.filter(x=>!x.erro&&x.valor>0).sort((a,b)=>a.valor-b.valor);
 return {resultados,melhor:validos[0]||null,configuracao:{dr:c.dr||null,servicos:c.servicos,servicosDetalhes:c.servicosDetalhes,fonte:c.descoberta?'API Meu Contrato':'Variáveis Vercel',aviso:c.aviso||null,modoServicos:c.modoServicos||null}};
}
...
module.exports={cfg,token,gerarToken,api,apiComToken,descobrirContrato,configuracaoEfetiva,cotar,rastrear,listarPrepostagens,listaPrepostResposta,valorProfundo,descobrirCodigoObjeto};
