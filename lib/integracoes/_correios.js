...
async function configuracaoEfetiva(){
  ...
  // V66: une os serviços configurados manualmente com os descobertos no contrato.
  // Antes CORREIOS_SERVICOS substituía a lista automática e podia fazer PAC ou SEDEX desaparecer.
  // Também tentamos sempre os dois serviços nacionais mais usados para comparação; se o contrato/CEP
  // não aceitar algum deles, a API devolverá erro apenas para aquela modalidade.
  const servicos=[...new Set([...(c.servicos||[]),...autoServicos,'03298','03220'])];
...
async function cotar({cepDestino,pesoKg,comprimento=20,largura=20,altura=20,servicos}){
 ...
 for(const coProduto of [...new Set(lista)]){
   resultados.push(await consultarServico(coProduto));
 }
