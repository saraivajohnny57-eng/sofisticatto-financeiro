...
async function rastrear(codigo){
 ...
 // V105: alguns tokens/escopos podem devolver o objeto sem o histórico de eventos.
 // Tenta cartão, contrato e usuário, e prefere a primeira resposta que realmente contenha eventos.
 const escopos=['cartao','contrato','user'];
 let ultimoErro=null,melhorResposta=null;
 for(const escopo of escopos){
   for(const t of requisicoes){
     try{
       const d=await apiComToken(`${base}${t.qs?'?'+t.qs:''}`,{headers:t.headers},escopo);
       if(!melhorResposta)melhorResposta=d;
       if(temEventosRastro(d))return d;
     }catch(e){
       ultimoErro=e;
       const msg=String(e?.message||'');
       // Falha de um escopo não impede testar outro. Erros de parâmetro também seguem para a próxima variante.
 ...
 // Importante: devolve a melhor resposta mesmo sem eventos para permitir diagnóstico,
 // mas o chamador NÃO deve rebaixar o status do pedido nesse caso.
