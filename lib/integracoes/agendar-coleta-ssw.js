...
async function descobrirConvite(transportadoraId,conviteId){
  if(conviteId)return conviteId;
  ...
  if(!c?.[0]?.id)throw new Error(`Não encontrei a integração da transportadora ${nome} no Portal de Integrações.`);
  return c[0].id;
}
...
module.exports=async function(req,res){
  ...
  try{
    const e=req.body||{};
    const conviteId=await descobrirConvite(e.transportadora_id,e.convite_id);
    const ambiente=e.ambiente||'producao';
    let cred={};
    try{cred=await obterCredenciaisIntegracao(conviteId,ambiente)}catch(err){
      // Compatibilidade: a ACCERT já utilizava variáveis Vercel na V37.
      // O Portal é a fonte principal; o fallback evita interromper a operação existente.
      const nomeTmp=(await supabaseRest('integracao_convites',{query:`?select=transportadora_nome&id=eq.${encodeURIComponent(conviteId)}&limit=1`}))?.[0]?.transportadora_nome||'';
      if(/accert/i.test(nomeTmp) && process.env.ACCERT_SSW_LOGIN && process.env.ACCERT_SSW_SENHA){
        cred={ssw_dominio:process.env.ACCERT_SSW_DOMINIO,username:process.env.ACCERT_SSW_LOGIN,password:process.env.ACCERT_SSW_SENHA};
      }else throw err;
    }
    const dominio=String(cred.ssw_dominio||cred.dominio||'').trim();
    const login=String(cred.username||cred.login||'').trim();
    const senha=String(cred.password||cred.senha||'');
    ...
    await supabaseRest('integracao_logs',{method:'POST',body:{convite_id:conviteId,transportadora_nome:e.transportadora_nome||null,operacao:'coleta',ambiente, http_status:r.status,tempo_ms:null,sucesso:true,mensagem:`Coleta SSW criada${numeroColeta?` — ${numeroColeta}`:''}`,created_at:agora}}).catch(()=>{});
