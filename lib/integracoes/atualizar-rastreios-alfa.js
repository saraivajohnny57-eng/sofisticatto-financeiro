  ...
  try{
    ...
    for(const x of (lista||[])){
      try{
        ...
        const c=await consultarAlfa({numeroNfe:x.numero_nfe,cnpj:"05451985000195"});
        const agora=new Date().toISOString();
        const patch={status:c.status,status_api:c.statusBruto,numero_cte:c.numeroCte||x.numero_cte||null,
          previsao_entrega:c.previsaoEntrega||x.previsao_entrega||null,ultima_ocorrencia:c.ultimaOcorrencia||null,
          ultima_ocorrencia_em:c.ultimaOcorrenciaEm||null,consulta_api:{retorno:c.dados,metodo_consulta:c.metodoConsulta,
          valor_consultado:c.valorConsultado,comprovante_url:c.comprovanteUrl,recebedor:c.recebedor,data_entrega:c.dataEntrega},
          metodo_consulta:c.metodoConsulta,consultado_api_em:agora,sincronizacao_erro:null,atualizado_em:agora,atualizado_por:"sincronizacao_alfa"};
        ...
        itens.push({id:x.id,nf:x.numero_nfe,anterior:antes,atual:c.status});
      }catch(e){erros++;itens.push({id:x.id,nf:x.numero_nfe,erro:e.message});}
