...
const {rastrearDetalhado,descobrirCodigoObjeto}=require('./_correios');
function normalizar(d){
  ...
  const eventos=listaOrdenada.map(ev=>{
    ...
    const destino=ev?.unidadeDestino||ev?.destino||ev?.unidade_destino||{};
    const endDestino=destino?.endereco||{};
    const local=[endereco?.cidade||unidade?.cidade||unidade?.nome||ev?.cidade,endereco?.uf||unidade?.uf||ev?.uf].filter(Boolean).join(' / ');
    const localDestino=[endDestino?.cidade||destino?.cidade||destino?.nome,endDestino?.uf||destino?.uf].filter(Boolean).join(' / ');
    return {
      titulo:tituloHumano,
      descricao:detalheEv||((descEv&&descEv!==tituloHumano)?descEv:'')||tipoEv||'',
      data:ev?.dtHrCriado||ev?.dataHora||ev?.data||null,
      local:local||null,
      destino:localDestino||null,
      unidade:unidade?.nome||unidade?.tipo||null,
...
module.exports=async(req,res)=>{if(req.method!=='GET')return json(res,405,{ok:false,erro:'Método não permitido.'});if(!exigirAdmin(req,res))return;try{
  ...
  const consulta=await rastrearDetalhado(codigo);
  const d=consulta.dados;
  const diagnostico=consulta.diagnostico||null;
